...
void lcd_buffer_add_next(lcd_buffer_s *buffer, uint16_t value)
{
    ...
    buffer->tail = (buffer->tail + 1) % BITS_ON_SCREEN;
}
...
void lcd_buffer_add_previous(lcd_buffer_s *buffer, uint16_t value)
{
    ...
    buffer->tail = (buffer->tail - 1 + BITS_ON_SCREEN) % BITS_ON_SCREEN;
}
...
int lcd_buffer_read_from_tail(lcd_buffer_s *buffer, int index)
{
	int cnt = ( (buffer->tail) + index) % BITS_ON_SCREEN;
    int result = buffer->data[cnt];
